"""netgraph CLI — parse, visualize, and lint vendor network configs.

Usage:
    netgraph map  ./router.cfg --lint
    netgraph map  ./router.cfg --os eos --out both --output-dir /tmp
    netgraph info ./router.cfg
"""
...
from netgraph.parsers.base import ParseError
...
def main() -> None:
    """netgraph — local-first network config dependency mapper."""
...
def cmd_map(config_file: Path, os_type, output_format, output_dir, lint, lint_severity):
    ...
    from netgraph.graph import GraphBuilder, HTMLExporter, JSONExporter
    from netgraph.analysis import DependencyResolver
...
def cmd_info(config_file: Path, os_type):
    ...
    if dangling or orphaned:
        ...
        click.echo("  Run with: netgraph map --lint  for details")
    else:
